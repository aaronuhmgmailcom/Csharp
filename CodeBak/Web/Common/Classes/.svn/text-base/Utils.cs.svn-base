...
namespace eChartProject.Web.Common
{
    ...
    public class Utils
    {
        public const string ASSEMBLY_VERSION = "1.1.0";
        ...
        /// GetInArrayID
        /// </summary>
        /// <param name="strSearch">input string</param>
        /// <param name="stringArray">string 's array</param>
        /// <param name="caseInsensetive">if care caseInsensetive</param>
        /// <returns>position of array</returns>
        ...
        /// qudiao回车换行符为html换行符
        /// </summary>
        public static string StrFormatD(string str)
        {
            string str2;
        ...
        /// <summary>
        /// convert invalid character
        /// </summary>
        /// <param name="content"></param>
        /// <returns></returns>
        public static string ConvertInvaidCharacter(string content)
        {
            ...
            content = Regex.Replace(content, @" ", "+", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\""", "\"\"", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"%", "%25", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @":", "%3A", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\*", "%2A", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\?", "%3F", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\[", "%5B", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\]", "%5D", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\|", "%7C", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\\", "%5C", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @"\/", "%2F", RegexOptions.IgnoreCase);
            content = Regex.Replace(content, @",", "%2C", RegexOptions.IgnoreCase);
            //content = Regex.Replace(content, @"\.", ".+", RegexOptions.IgnoreCase);
            //content = Regex.Replace(content, @"\)", ")+", RegexOptions.IgnoreCase);
    ...
    public class Robot
    {
        ...
        /// </summary>
        /// <param name="str"></param>
        public static string QUERYROBOT(string str)
        {
            string strRobotURI = ConfigurationManager.AppSettings.Get("RobotURI");
            Uri uri = new Uri(strRobotURI + "");
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(uri + "?" + OpenStr(str));
        ...
        /// <param name="fileName"></param>
        /// <returns></returns>
        public static string OpenStr(string input)
        {
            string str = "q=" + input;
            return str;
        }
        /// <summary>
        /// Save MSG ANSWER TO REMOTE Robot
        /// </summary>
        /// <param name="dt"></param>
        public static void SAVETOROBOT(DataTable dt)
        {
            SaveCSV(dt, System.Web.HttpRuntime.AppDomainAppPath + "Book1.CSV");

            string strRobotURI = ConfigurationManager.AppSettings.Get("RobotURI");

            Uri uri = new Uri(strRobotURI+"");

            byte[] postData = Encoding.UTF8.GetBytes(OpenCSV(System.Web.HttpRuntime.AppDomainAppPath + "Book1.CSV"));
        ...
        public static string OpenCSV(string fileName)
        {
            FileStream fs = new FileStream(fileName, System.IO.FileMode.Open, System.IO.FileAccess.Read);
            StreamReader sr = new StreamReader(fs, System.Text.Encoding.UTF8);
            string str = "u=" + sr.ReadToEnd();
            sr.Close();
            fs.Close();
            return str;
        }
