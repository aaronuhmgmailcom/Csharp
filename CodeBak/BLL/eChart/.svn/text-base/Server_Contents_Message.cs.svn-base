...
namespace eChartProject.BLL.eChart
{
	...
	public partial class server_contents_message
	{
        ...
        {
            return dal.UpdateByIsPublic(model);
        }
        /// <summary>
        /// 更新一条数据(只更新ISDELETED)
        /// </summary>
		...
		public List<eChartProject.Model.eChart.server_contents_message> DataTableToList(DataTable dt)
		{
			...
			if (rowsCount > 0)
			{
				...
				for (int n = 0; n < rowsCount; n++)
				{
					...
					{
                        model.isOffLine = dt.Rows[n]["isOffLine"].ToString() == "TRUE" ? 1 : 0;
					}
					if(dt.Rows[n]["isPublic"]!=null && dt.Rows[n]["isPublic"].ToString()!="")
					{
                        model.isPublic = dt.Rows[n]["isPublic"].ToString() == "TRUE" ? 1 : 0;
					}
					if(dt.Rows[n]["isVariations"]!=null && dt.Rows[n]["isVariations"].ToString()!="")
					{
                        model.isVariations = dt.Rows[n]["isVariations"].ToString() == "TRUE" ? 1 : 0;
					}
					...
					{
                        model.isDeleted = dt.Rows[n]["isDeleted"].ToString() == "TRUE" ? 1 : 0;
					}
					modelList.Add(model);
				}
			}
